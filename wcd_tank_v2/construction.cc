...
{
    // Crear instancia del detector sensible y registrarla en el SDManager
...
//----------------------------------------------------------------------------------------	
	...
	G4PVPlacement(rotation, //se aplica misma rotacion que el estanque 
		G4ThreeVector(0,0.9*m,0), logicDetector,"physDetector",logicWorld, false, 0, true);
		//
	//
...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
    ...
    // Definir materiales y geometría como lo has hecho antes...
    
    // Crear el volumen del detector (ya existente)
    G4Tubs *solidDetector = new G4Tubs("solidDetector", 0., 0.15*m, 0.1*m,0., 2 * pi);
    logicDetector = new G4LogicalVolume(solidDetector, nist->FindOrBuildMaterial("G4_WATER"), "logicDetector");
